...
def get_vanilla_vgg_features(cut_idx=-1):
    if not os.path.exists('vgg_features.pth'):
        ...
        vgg_weights = torch.load('vgg19-d01eb7cb.pth', map_location='cpu')
        ...
        model.classifier = nn.Sequential(View(), *list(model.classifier._modules.values()))
    ...
    vgg = torch.load('vgg_features.pth', map_location='cpu')
    if cut_idx > 36:
        vgg_classifier = torch.load('vgg_classifier.pth', map_location='cpu')
        vgg = nn.Sequential(*(list(vgg._modules.values()) + list(vgg_classifier._modules.values())))
...
def get_vgg(cut_idx=-1):
    ...
    if cut_idx > 0: 
        ...
        keys = list(f._modules.keys())
        keys_to_delete = [keys[x] for x in range(cut_idx, num_modules)]
...
def vgg_preprocess_var(var):
        ...
        mean = vgg_mean[None, ...].to(dtype=var.dtype, device=var.device)
        out = bgr * 255 - mean.expand_as(bgr)
        return out
