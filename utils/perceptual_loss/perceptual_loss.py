...
def get_pretrained_net(name):
    ...
    if name == 'alexnet_caffe':
        ...
        model = VGGModified(vgg19(weights=None), 0.2)
        model.load_state_dict(torch.load('vgg_pytorch_modified.pkl', map_location='cpu')['state_dict'])
...
class PerceputalLoss(nn.modules.loss._Loss):
    ...
    def __init__(self, input_range='sigmoid', 
        ...
        self.net = get_pretrained_net(net_type).to(self.device)
...
def get_vgg19_caffe():
    ...
    model.load_state_dict(torch.load('vgg19-caffe-py3.pth', map_location='cpu'))

    return model

def get_vgg16_caffe():
    vgg = torch.load('vgg16-caffe-py3.pth', map_location='cpu')
...
def vgg_preprocess_caffe(var):
    ...
    out = bgr * 255 - vgg_mean.to(dtype=var.dtype, device=var.device)
    return out



mean_pytorch = torch.Tensor([0.485, 0.456, 0.406]).view(3, 1, 1)
std_pytorch = torch.Tensor([0.229, 0.224, 0.225]).view(3, 1, 1)
